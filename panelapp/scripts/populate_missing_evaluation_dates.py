...
with open(sys.argv[1], 'r') as f:
    ...
    for row in reader:
        ...
        missing_data[row[0]][row[1]][row[2]] = {
            'version': row[3],
            'date': row[4]
        }
...
with transaction.atomic():
    old_pks = GenePanelSnapshot.objects.get_latest_ids(deleted=True).filter(panel__old_pk__in=missing_data.keys())
    old_gps = GenePanelSnapshot.objects.filter(pk__in=old_pks).prefetch_related('level4title')
    print('Backing up the panels')
    for gps in old_gps:
        gps.increment_version()
        print('Backed up {}'.format(gps))

    new_pks = GenePanelSnapshot.objects.get_latest_ids(deleted=True).filter(panel__old_pk__in=missing_data.keys())
